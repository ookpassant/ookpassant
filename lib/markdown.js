...
// a paragraph that is nothing but [video: <youtube url or id> | caption] becomes
...
function videos(html) {
  return html.replace(VIDEO, (whole, inner) => {
    ...
    const title = caption.replace(/&[a-z]+;|["<>]/gi, ' ').trim() || 'embedded video';
...
function render(body) {
  return videos(plates(md.render(body)));
}
