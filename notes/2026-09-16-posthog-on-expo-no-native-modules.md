...
Leave the first three out. The SDK feature-detects its way around them. Persistence falls back to AsyncStorage, device context comes from `expo-constants` and `Platform`, and error tracking stays at the JavaScript level. You give up very little, and an over-the-air update can never land on a binary whose native side disagrees with it.
...
Construction sets up the queue and the AppState listeners, so even that gets a try. Analytics is never worth a boot crash.

`captureAppLifecycleEvents: true` gives you Application Opened, Backgrounded, Installed and Updated for free. Those are the only free events in this setup. Everything else you write by hand.

## Events mark a decision or a physical act, never a screen

With no provider there's no expo-router integration and no `$screen` event. All 72 of Whimsee's events are hand-written `track()` calls.[^2] That's a real trade and you should make it on purpose. You give up autocapture funnels entirely, and you get an event stream where nothing arrives that you didn't choose to send, which matters when a screen name would itself tell me something about a person I don't want to know.
...
That one has numbers. In the last 90 days `photo_rejected` fired twice. Two refusals against 80 photo glimmers successfully planted, a rejection rate of 2.4%, and both landed before `photo_compression` went live on 12 August. In the five weeks since, the app has shrunk 46 photos and refused none. Ten call sites and two events, which between them show a fix working.
...
Whimsee is a location app, and no event it sends to analytics carries a precise coordinate. Every event that needs a place carries `area`, and `area` is a five-character geohash: a cell about 4.9km on a side. That is still location information, and I treat it as such. It's enough to say this part of the Forest is busy and too blunt to point at a house.

```ts
track('glimmer_found', { area: coarseArea(lat, lon), content_type, offline, again });
```

The geohash is computed on the phone before anything leaves it, and it's standard geohash so it agrees with every other implementation.[^3] Content never travels either. `glimmer_found` carries what type of thing was found and whether it was found offline, enough to read a walk as a story, nothing that says what was found or where.

## Fire the event when the thing happens, not when it syncs

I found this one while writing the post.
...
I don't know which of two things that is. Either the dead zones are less total than I think and people really are finding things in signal, or there's a second version of the plant-side bug sitting somewhere on the hunt side and I haven't found it. That's the difference between a feature nobody needs and a feature nobody can reach, and I can't tell them apart from a dashboard. So it goes down as unresolved.
...
Nothing crashed. The SDK feature-detects in both directions. What it means is that the first launch of a store build reads an empty store, so it regenerates the anonymous ID and runs on flag fallbacks until the first fetch comes back. I didn't watch that happen; I'm reading it off the storage picker.[^4] The comment I'd left in `analytics.ts` saying `'file'` picks the best available persistence, "here AsyncStorage", was wrong.
...
What protected a child was the general architecture: no content in any event, a 4.9km cell rather than a precise coordinate, no ad SDKs, pseudonymous identity, EU cloud. That's more than most, and it's still not the same as treating a child differently, and the ICO's Age Appropriate Design Code treats profiling off by default for children as a standard rather than a nicety.
...
And the honest bit. Whimsee has disciplined instrumentation and almost no readout. 72 carefully designed events and, at the last audit, 1 of them wired to a decision anyone was making, which is 1.4%. In the 30 days to 15 September, the funnel from `hunt_started` to `dig_zone_arrived` ran 44.3% to 5.1%,[^5] down at both steps since July, which looks like a catastrophe and is actually expected, because there aren't enough glimmers in the ground yet for most hunts to end in a dig. The bottom of that funnel is the part worth pointing at: of the four people who reached the dig zone, all four revealed a glimmer, and all four went on to decide whether to keep it. Median time from arriving to revealing, thirty seconds. The entire loss is in getting there.

I'd still build it this way. You can't reconstruct what accuracy a phone had last September, and you can't reconstruct which plants were made under trees either. Now you can.
